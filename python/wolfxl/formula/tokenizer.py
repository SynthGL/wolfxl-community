"""Vendored openpyxl 3.1.5 formula tokenizer.

The wolfxl-native Tokenizer was a simplified port that did not expose
``make_subexp``, ``make_separator``, ``get_closer``, ``save_token``,
or ``assert_empty_token`` — the openpyxl test corpus exercises every
one of those entry points. Vendoring upstream verbatim is the cheapest
way to clear that 177-test cluster; ``translate.py``'s only use of
this module is ``Tokenizer(formula).items`` plus ``Token.type``/
``subtype``/``value``, which are preserved.

The tokenizer is based on the Javascript tokenizer found at
http://ewbi.blogs.com/develops/2004/12/excel_formula_p.html written by
Eric Bachtal.
"""
...
__all__ = ["Token", "Tokenizer", "TokenizerError", "re"]
