...
impl Worksheet {
    ...
    /// Remove a pending merge by A1 range string.
    pub fn unmerge_cells(&mut self, range: &str) -> Result<(), String> {
        let ((r1, c1), (r2, c2)) =
            refs::parse_range(range).ok_or_else(|| format!("invalid A1 range: {range:?}"))?;
...
pub struct Hyperlink {
    ...
    pub target: String,
    /// Optional external-location/fragment attribute paired with an external
    /// relationship target. Openpyxl preserves this as ``Hyperlink.location``
    /// even when ``target`` points at a URL.
...
mod tests {
    ...
    #[test]
    fn unmerge_cells_removes_existing_range() {
        let mut s = Worksheet::new("S");
        s.merge_cells("A1:B2").unwrap();
        s.merge_cells("D1:E1").unwrap();

        assert!(s.unmerge_cells("A1:B2").is_ok());
